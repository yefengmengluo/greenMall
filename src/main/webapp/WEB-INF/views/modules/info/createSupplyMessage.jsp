      ...
      $.ajax({
        ...
        success:function(data) {
          if(data){
            ...
            alert("新增保存成功！")
//            location.reload()
              <c:if test="${infoMessage.isEntrust eq '1'}">
              window.location.href="${pageContext.request.contextPath}${fns:getAdminPath()}/info/msgEntrustInfoes";
              </c:if>
              <c:if test="${infoMessage.isEntrust ne '1'}">
              window.location.href="${pageContext.request.contextPath}${fns:getAdminPath()}/info/supplyMessageCheck";
              </c:if>
          }
        }
      })
...
<div>
  ...
  <div>
    <table  class="table table-striped table-bordered table-condensed">
      <thead><th>状态</th><th>信息编号</th><th>信息</th>
        <%--<th>备注</th>--%>
        <c:if test="${infoMessage.isEntrust ne '1'}">
      <th>操作</th>
        </c:if>
      ...
      <tbody>
        ...
        <c:if test="${infoMessage.isEntrust ne '1'}">
          <td>
            <a href="${ctx}/infoMessage/checkSupplyMessage?id=${infoMessage.id}">编辑</a>
            &nbsp;&nbsp;&nbsp;&nbsp;
    ...
    <c:if test="${infoMessage.isEntrust eq '1'}">
  <h3>被委托的${infoMessage.type eq 'supply'?'求购':'供应'}信息</h3>
  <div>
    <table  class="table table-striped table-bordered table-condensed">
      ...
      <%--</tbody>--%>
        <tr>
            <td>商品类别：${entrustInfo.pgoodsName}</td>
            <td>联系人：${entrustInfo.userName}</td>
        </tr>
          <tr>
            <td>品种：${entrustInfo.goodsName}</td>
            <td>手机：${entrustInfo.telephone}</td>
        </tr>
          <tr>
            <td>规格：${entrustInfo.checkedSpecsId}</td>
              <td>信息状态：${fns:getDictLabel(entrustInfo.statue, 'supply_demand_info_statue', defaultValue)}</td>
        </tr>
          <tr>
            <td>${entrustInfo.type eq 'supply'?'供应':'采购'}量：${entrustInfo.number}${entrustInfo.numberUnitValue}&nbsp;&nbsp;${entrustInfo.nonstandardNumber}${entrustInfo.nonstandardNumberUnitName}</td>
              <td>有效期：${fns:formatDate(entrustInfo.validateStartDate,'yyyy-MM-dd')}至${fns:formatDate(entrustInfo.validateEndDate,'yyyy-MM-dd')}</td>
        </tr>
          <tr>
            <td>价格：${entrustInfo.fromPerPrice}${not entrustInfo.fromPerPrice && not empty entrustInfo.toPerPrice ?'-':''}${entrustInfo.toPerPrice}${empty entrustInfo.fromPerPrice && empty entrustInfo.toPerPrice ?'面议':entrustInfo.priceUnitValue}</td>

              <td>备注：${entrustInfo.remarks}</td>
        </tr>
          <tr>
            <td>货源地：${entrustInfo.productionProvinceName}${entrustInfo.productionCityName}${entrustInfo.productionAreaName}</td>

              <td>发货地址：${entrustInfo.provinceName}${entrustInfo.cityName}${entrustInfo.areaName}</td>
        </tr>
          <tr>
            <td>货源详细地址：${entrustInfo.productionDetailArea}</td>
              <td>详细发货地址：${entrustInfo.detailArea}</td>
        </tr>
    </table>
...
<br>
<div id="infoDiv">
    <h3>审核添加${infoMessage.type eq 'supply'?'供应':'求购'}信息</h3>
