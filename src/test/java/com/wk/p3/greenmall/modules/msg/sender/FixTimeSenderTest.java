...
public class FixTimeSenderTest {
     ...
     */
    @Test
    public void sendMessageFixTime() {
        //设置固定时间发送消息，时间为当前时间延后10秒
        Date data = new Date(System.currentTimeMillis()+1000*delay);
        try {
            messageService.sendMessage(new FixTimeSender(data),new FreeMarkerMsg(),target, new DeliverInterceptor() {
                @Override
    ...
    public static class TargetForTest extends TargetInterceptor implements Serializable {
        ...
        public void send0(Msg msg) throws SendException {
            ...
            Assert.assertTrue(map.get("toSendTime").getSeconds() - map.get("deliverTime").getSeconds() == delay);
