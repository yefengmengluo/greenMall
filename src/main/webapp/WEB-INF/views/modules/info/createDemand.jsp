...
//      $("#specs").val(specs)
      ...
      //publishUser
      var publishUserId = $("#publishUserId option:selected").val()
      if(!publishUserId){
        alert("发布人不能为空")
        return;
      }
      $.ajax({
        ...
        success:function(data) {
          if(data){
            ...
            alert("新增保存成功！")
//            location.reload()
             window.location.href="${pageContext.request.contextPath}${fns:getAdminPath()}/info/createDemand";
          }
        }
      })
      $("#saveEditForm").submit()
...
<table id="contentTable" class="table table-striped table-bordered table-condensed">
    ...
    <td>
      ...
      <div  id="showGoodsSelectDiv" style="position: absolute;width: 28%;top: 0px;left: 0px;background-color:#cc9966;display:none;">
        ...
        <div style="margin-left: 20px;">
          ...
          <input type="button" value="确认" onclick="writeOtherGoods()" style="display:none" style="display:none">
        </div>
      </div>
    </td>
  </tr>
  ...
  <tr>
    <td>发布人</td>
    <td>
      <select id="publishUserId" style="width: 200px;">
        <c:forEach items="${publishUsers}" var="publishUser">
          <option value="${publishUser.id}">${publishUser.name}-${publishUser.mobile}</option>
        </c:forEach>
      </select>
    </td>
  </tr>
