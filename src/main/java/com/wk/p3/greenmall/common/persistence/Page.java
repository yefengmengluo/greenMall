...
public class Page<T> {
	...
	private List<Integer> showPages;//展示的页面索引集合,暂定5个
...
//	}
	...
	public int getPageCount() {
		...
		return i % pageSize > 0 ? (i / pageSize) + 1 : i / pageSize;
	}
	...
	public List<Integer> getShowPages() {
		...
		if(getPageCount() == 0){
		}else if(getPageCount()>0 && getPageCount()<6){
			for(Integer i=0;i<getPageCount();i++){
				list.add(i+1);
			}
		}else{
			if((getPageCount()-getPageNo())>=2){
				if(getPageNo()-2>0){
					list.add(getPageNo()-2);
					list.add(getPageNo()-1);
					list.add(getPageNo());
					list.add(getPageNo()+1);
					list.add(getPageNo()+2);
				}else{
			...
			}else if((getPageCount()-getPageNo())<2){
				list.add(getPageCount()-4);
				list.add(getPageCount()-3);
				list.add(getPageCount()-2);
				list.add(getPageCount()-1);
				list.add(getPageCount());
			}
		}
		return list;
	}
